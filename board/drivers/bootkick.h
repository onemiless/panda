...
void bootkick_debug_restore(void) {
  const uint16_t persisted_debug_wait = (uint16_t)(wake_debug.can_exti_line >> 16U);
  if (persisted_debug_wait > 0U) {
...
void bootkick_debug_schedule(uint16_t delay_s) {
  ...
  wake_debug_bootkick_schedule(debug_bootkick_countdown);
}
